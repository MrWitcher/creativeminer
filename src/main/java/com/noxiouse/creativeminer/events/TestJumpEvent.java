...
*/
    ...
    public static void detectOre(LivingEvent.LivingUpdateEvent event){
        ...
        BlockState chunks = chunk.getBlockState(world.rayTraceBlocks(new RayTraceContext(start, start.add(end), RayTraceContext.BlockMode.OUTLINE, RayTraceContext.FluidMode.NONE, livingEvent.getEntity())).getPos());

            if(chunks.getBlock().matchesBlock(Blocks.IRON_ORE)){
                CreativeMiner.LOGGER.info(detect);
                world.destroyBlock(world.rayTraceBlocks(new RayTraceContext(start,start.add(end), RayTraceContext.BlockMode.OUTLINE, RayTraceContext.FluidMode.NONE,livingEvent.getEntity())).getPos(),true);
            }
    }
}